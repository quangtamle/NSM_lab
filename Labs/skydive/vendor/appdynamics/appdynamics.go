...
func StartCall(src *Tier, dst *Tier, callName string) error {
    call := Call{
        Src: src,
        Dst: dst,
        Name: callName,
    ...
    if _,ok := Agent.Tiers[src.Name]; !ok {
        return errors.New("Need to update agent")
    }
    if _,ok := Agent.Tiers[dst.Name]; !ok {
        return errors.New("Need to update agent")
    }

    if src.Backend {
        return errors.New("Could not start call from Backend")
    }


    AddBackend(dst.Name, callName, map[string]string{}, true)

    call.BTsrc = StartBTWithAppContext(src.Name,src.Name + "/" + callName + "/" + dst.Name, "")

    fmt.Println("Start call from : " + src.Name + " to : " + dst.Name)

    call.ExitCall = StartExitcall(call.BTsrc, dst.Name)

    if !dst.Backend {
        hdr := GetExitcallCorrelationHeader(call.ExitCall)
        call.BTdst = StartBTWithAppContext(dst.Name,src.Name + "/" + callName + "/" + dst.Name,hdr)
    }

    return nil
}

func StopCall (src *Tier, dst *Tier, callName string) { 
    newCalls := make([]*Call,0)
    for _,call := range Agent.Calls {
        if call.Src.Name == src.Name {
            if call.Dst.Name == dst.Name {
                if callName == call.Name {
                    if !dst.Backend {
                        EndBT(call.BTdst)
                    }
                    EndExitcall(call.ExitCall)
                    EndBT(call.BTsrc)
...
func Update () error {
    TerminateSDK()
    cfg := Agent.Config
    for _, tier := range Agent.Tiers {
        contextConfig := &ContextConfig{
            AppName:  cfg.AppName,
            TierName: tier.Name,
            NodeName: tier.Name,
        }
        if err := AddAppContextToConfig(cfg, tier.Name, contextConfig); err != nil {
            return err
        }
    }
...
func AddTier (name string, backend bool) *Tier {
    ...
    if Agent.Tiers == nil {
        ...
        Agent.Tiers = make(map[string]*Tier,0)
        if !backend {
            Update()
        }
    }

    Agent.Tiers[name] = newtier
    if !backend {
        Update()
    }
    return newtier
